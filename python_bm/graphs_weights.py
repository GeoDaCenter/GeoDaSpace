...
plt.plot(n_r[:len(creWe_r)], creWe_r, label='R')
plt.plot(n_py[:len(creWe_py)], creWe_py, label='Spreg')
plt.legend(loc=2)
plt.title('Computation time to create lattice (queen) weights', weight='bold')
plt.xlabel('N')
plt.ylabel('Seconds')

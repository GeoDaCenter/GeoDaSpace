...
if __name__ == '__main__':
    ...
    for j in range(len(y)):
        if y[j] > 50:
            yp[j]=1
    probit1=probit(X,yp,w=w)
    print "LM Error - Pinkse (1999):", probit1.LM_error
    print "Moran's I - KP (2001):", probit1.moran
    print "LM Error - PS (1998):", probit1.pinkse_slade
    '''
    import numpy as np
    import pysal
    db = pysal.open("examples/greene21_1.csv",'r')
    ...
    for i in var['x']:
        X.append(db.by_col(i))
    X = np.array(X).T
    probit1=probit(X,y,scalem='phimean')
    print "Parameters:"
    print probit1.betas
    print "Std-Dev:"

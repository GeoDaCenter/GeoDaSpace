...
#
    ...
    areNans = True
#
    while areNans and attempts < 100:
        attempts += 1
        print '\nAttempt %i\n'% attempts
        print cols_to_replace
        for col in cols_to_replace:
            U[:, col] = np.random.uniform(0, 1, (n, ))
        ...
        #
        print 'BAD ', len(np.where(np.isnan(z))[0])
        cols_to_replace.update(np.where(np.isnan(z))[1])
        cols_to_replace.update(np.where(np.isinf(z))[1])
    ...
    meanT=np.mean(T)
    if np.isnan(np.log(meanT)) or np.isinf(np.log(meanT)):
        print z
    #print zj.T

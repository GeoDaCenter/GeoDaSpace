...
class TextWindow(view.xrcTextWindow, StringIO.StringIO):
    ...
    def bindings(self):
        ...
        self.Bind(wx.EVT_MENU, self.smaller,
                  id=wx.xrc.XRCID("ToolFontSmaller"))
    ...
    def confirm(self):
        """
        Prevents results from being lost be promting the user to save
        any changes that have been made, returns True, ok to continue
        or False, NOT ok to continue, if false do not distrube any results.
        """
        if self.Text.IsModified() or "*" in self.GetTitle():
            # * in title is a bug fix for windows,
            # somehow Modified gets set to false!
            if not self.IsShown():
                self.Show()
            self.Raise()
            confirmDialog = wx.MessageDialog(
                self, "Your changes will be lost if you don't save them.",
                'Do you want to save the changes you made in "%s"?' %
                self.GetTitle().replace('*', ''),
    ...
    def open(self, evt=None):
        if self.confirm():
            ...
            fileDialog = wx.FileDialog(
                self, message="Select A File To Open...", wildcard="*.txt")
    ...
    def saveAs(self, evt=None):
        fileDialog = wx.FileDialog(
            self, defaultFile=self.GetTitle().replace('*', '') + '.txt',
            message="Save File As...", wildcard="*.txt",

...
def w_regimes_dict(w, regimes, regimes_set, transform=True, get_ids=None, min_n=None):
    ...
    w_regi_i, warn = {},{}
    for r in regimes_set:
        w_regi_i[r] = pysal.weights.w_subset(w, w_ids[r], silent_island_warning=True)
        if min_n:
...
def w_regimes_union(w, w_regi_i, regimes_set):
    ...
    '''
    w_regi = pysal.weights.w_union(w_regi_i[regimes_set[0]], w_regi_i[regimes_set[1]], silent_island_warning=True)
    if len(regimes_set)>2:
        for i in range(len(regimes_set))[2:]:
            w_regi = pysal.weights.w_union(w_regi, w_regi_i[regimes_set[i]], silent_island_warning=True)

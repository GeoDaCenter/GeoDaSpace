...
DISTANCE_METRICS = ['Euclidean Distance',
                    'Arc Distance (miles)', 'Arc Distance (kilometers)']
...
class GeoDaSpace_W_Obj(object):
    ...
    def shapefile_hint(self):
        """
        Returns a suggested path to the associated shapefile.
        Path is only a hint and should be verified.
        """
        if self.has_meta:
    ...
    def name(self):
        if self.has_meta:
            ...
            if 'method options' in self.w.meta:
                ...
                if type(opts) == list:
                    name += ': ' + ', '.join(map(
                        str, self.w.meta['method options']))
                else:
                    name += ': ' + opts
...
class weightsModel(AbstractModel):
    ...
    @property
    def bbox_diag(self):
        """ Returns the length of the diagonal of the bounding box the
        selected shapefile or 0.0 """
        ...
        if self.shapes:
            if self.distMethod == 0:  # 'Euclidean Distance'
                ...
                x, y, X, Y = self.shapes.bbox
                self._propData['bbox_diag'] = pysal.cg.arcdist((
                    y, x), (Y, X), radius=pysal.cg.RADIUS_EARTH_MILES)
                return self._propData['bbox_diag']
            elif self.distMethod == 2:  # 'Arc Distance (kilometers)'
                x, y, X, Y = self.shapes.bbox
                self._propData['bbox_diag'] = pysal.cg.arcdist((
                    y, x), (Y, X), radius=pysal.cg.RADIUS_EARTH_KM)
                return self._propData['bbox_diag']
        return 0.0

    @property
    def knn1_dist(self):
        """ Returns the minimum distance required for each point to have at
        least one neighbor. This us the max dist returned by knn1
        (k=2 because scipy includes each point as it's own neighbor)
        """
        ...
        if 'kd' in self._propData:
            ...
            if pts is not None:
                if self.distMethod == 0:  # not Euclidean Distance
                    ...
                    kd = KDTree(pts, distance_metric="Arc",
    ...
    def points(self):
        """ Attempts to return an array of points
            If the selected shapefile contains polygons,
            the centroids will be returned.
        """
        if not self.shapes:
            return None
        shps = self.shapes

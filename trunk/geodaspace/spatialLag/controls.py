...
class C_spLagVariable(SpatialLag_xrc.xrcSpLagVariable):
    ...
    def __init__(self,parent=None):
        ...
        self.dispatch = d
        self.varsChoice.Bind(wx.EVT_CHOICE,self.__var)#AppendItems(self.model.get('vars'))
        self.newVarNameCtrl.Bind(wx.EVT_TEXT,self.__newVarName)#SetValue(newVarName)
        ...
        if evt is not None: #evt fired
            ...
            if type(value) == int:
                ...
                if value != -1 and old_value != value:
                    self.__newVarName(value=value)
                    #self.model.set('newVarName',self.model.get('vars')[self.model.get('var')])
            else: #assume list
    ...
    def __caution(self,evt=None,value=None):
        if value is not None:
            if self.model.get('newVarName') in self.model.get('vars'):
                self.warn.SetToolTipString("This variable already exists, and will be overwritten!")
                self.warn.Show()
                self.model.set('caution',True,True)
            elif len(self.model.get('newVarName')) < 1 and self.newVarNameCtrl.IsEnabled():
                self.warn.SetToolTipString("Please enter a variable name, or leave empty to exclude this variable.")
                self.warn.Show()
                self.model.set('caution',True,True)
            elif self.model.newVars.count(self.model.get('newVarName')) > 1:
                self.warn.SetToolTipString("You are creating another new variable with this name, please change one of them!")
                self.warn.Show()
                self.model.set('caution',True,True)
            elif len(self.model.get('newVarName')) > 10:
                self.warn.SetToolTipString("This variable name is too long, it will be truncated to 10 chars.")
                self.warn.Show()
                self.model.set('caution',True,True)
            else:
                self.warn.Hide()
...
class C_CreateSpatialLag(SpatialLag_xrc.xrcCreateSpatialLag):
    def __init__(self,parent=None,dataFile=None,wtFiles=[''],vars=[''],results=[],dialogMode=False):
        ...
        #hack to force initial data file to show up in GUI, even if it doesn't exist
        self.model.data['dataFile'] = dataFile
        self.populate(None)
        self.model.set('dataFile',dataFile)

        #VariablesPeer is a hidden panel of size (0,0), this is necessary because XRC does
        #not allow accessing sizers directly, so you get call a sizer item's GetContainingSizer method
    ...
    def run(self,evt):
        ...
        wild = '*.'+dfile.split('.')[1]
        fileDialog = wx.FileDialog(self,defaultFile=dfile,defaultDir=dpath,message="Save Data File As...",wildcard=wild,style=wx.SAVE+wx.OVERWRITE_PROMPT)
        result = fileDialog.ShowModal()
        if result == wx.ID_OK:
            path = fileDialog.GetPath()
            self.model.run(path)
            self.results.append(path)
            dialog = wx.MessageDialog(self,"The new variables have been saved in,\n%s"%(path),"Success",wx.OK|wx.ICON_INFORMATION)
            dialog.ShowModal()
    ...
    def __wtFile(self,evt=None,value=None):
        if evt:
            if evt.EventType == wx.EVT_CHOICE.typeId: # The drop down
                ...
                fileDialog = wx.FileDialog(self,message="Choose Weights File",wildcard=filter)
                result = fileDialog.ShowModal()
                if result == wx.ID_OK:
                    path = fileDialog.GetPath()
    ...
    def __dataFile(self,evt=None,value=None):
        if evt:
            if evt.EventType == wx.EVT_BUTTON.typeId:
                filter = "Data File (*.dbf; *.csv)|*.dbf;*.csv" #"|*.gal|GWT file|*.gwt|XML Weights|*.xml"
                fileDialog = wx.FileDialog(self,message="Choose Data File",wildcard=filter)
                result = fileDialog.ShowModal()
                if result == wx.ID_OK:
                    path = fileDialog.GetPath()
                    self.model.set('dataFile',path)
                ...
                if not value == self.dataFile.GetValue():
                    self.model.set('dataFile',self.dataFile.GetValue())#,passive=True)
        elif value is not None:
            if value == False:
